...
<html><head><title>RetroShow</title>
...
<tr><td colspan="2">
  <b>Latest Videos From Users:</b><br>
  <?php
  ...
  $stmt = $db->query("SELECT id, title, user, preview FROM videos ORDER BY id DESC LIMIT $offset, $per_page");
  while ($row = $stmt->fetch()) {
    ...
    echo 'Creator: <a href="channel.php?user='.htmlspecialchars($row['user']).'">'.$row['user'].'</a></td>';
    echo '</tr></table>';
  }
